...
Where  InformationAboutFlowsheetGroupsOrRowsTable.FLO_MEAS_NAME  IN ('R PAIN SCORE', 'PAIN SCORE (CATEGORY LIST)') --This gives me the flow sheet for the pain score
	   ...
	   AND EncounterTable.PAT_ENC_CSN_ID IN (SELECT ENCOUNTERS1.PAT_ENC_CSN_ID Encounter_ID
...
SELECT     
  Billing_Provider,    
  EOMONTH(D0140D0150_SERVICEDATE) AS Service_Month,    
  COUNT (#PaintAssessment.Encounter_ID) AS Numerator,    
  COUNT (#EncounterPopulation.Encounter_ID) AS Denominator    
FROM #EncounterPopulation    
LEFT JOIN #PaintAssessment ON #PaintAssessment.Encounter_ID = #EncounterPopulation.Encounter_ID    
GROUP BY     
  Billing_Provider,    
  EOMONTH(D0140D0150_SERVICEDATE)   
ORDER BY Billing_Provider, Service_Month DESC  
